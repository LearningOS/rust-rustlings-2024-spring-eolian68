...
pub trait Graph {
    ...
    fn add_node(&mut self, node: &str) -> bool {
        //TODO
        let ret = self.contains(node);
        if !ret { self.adjacency_table_mutable().insert(node.to_string(), vec![(node.to_string(), 0)]);}
        ret
    }
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        //TODO
        if self.contains(edge.0) {
            println!("contains edge.0! {}", edge.0);
        ...
        if self.contains(edge.1) {
            println!("contains edge.1! {}", edge.1);
